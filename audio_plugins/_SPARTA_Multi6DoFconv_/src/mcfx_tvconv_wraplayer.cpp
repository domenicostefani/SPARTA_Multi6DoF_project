...
#endif
    juce::OwnedArray<ConvolverData>* convdata_s;  // Array with n Convolver data structures, each containing the filter data for a listener position
...
void mcfxConv_create(
    ...
    *phMCFXc = malloc1d(sizeof(Internal_Conv_struct));
    ...
    // Load the IR matrices in the way that the MtxConvMaster class expects through the ConvolverData class
    for (int listenerPosition = 0; listenerPosition < nIRs; listenerPosition++) {
...
#define NUM_DIMENSIONS (3)
    ...
    typedef struct _mcfxconv {
        ...
        // int hostBlockSize;         //Replaced by _BufferSize from the MCFX convolver//                   /*< current host block size */
        // int hostBlockSize_clamped; //No longer used since MCXF convolver does not clamp the block size// /**< Clamped between MIN and #MAX_FRAME_SIZE */
        // int host_fs;               //Replaced by _SampleRate from the MCFX convolver//                   /**< current samplerate of the host */
    ...
    void tvconv_init(
        ...
        if (pData->_BufferSize != hostBlockSize) {
            ...
            pData->_ConvBufferSize = hostBlockSize;
            pData->reInitFilters = 1;
    ...
    #if (CROSSFADE == CROSSFADE_RELEASE) && defined(CROSSFADE) && defined(CROSSFADE_RELEASE)
                        curXfdInfo->gain.applyGain(*curXfdOut, nSamples);
    #elif (CROSSFADE == CROSSFADE_DEBUG_MUTE) && defined(CROSSFADE) && defined(CROSSFADE_DEBUG_MUTE)
                        curXfdOut->applyGain(0.0);  // Set gain at 0.5 if crossfade is disabled
                        curXfdInfo->gain.skip(buffer.getNumSamples());
    #elif (CROSSFADE == CROSSFADE_DEBUG_DISABLE) && defined(CROSSFADE) && defined(CROSSFADE_DEBUG_DISABLE)
    ...
    void tvconv_checkReInit(void* const hMcfxConv) {
        ...
        if ((pData->reInitFilters == 1) && (pData->irs != NULL)) {
            ...
            if (pData->ir_length > 0) {
                bool checkAndNotifyResampling = (pData->ir_fs != (int)pData->_SampleRate);
